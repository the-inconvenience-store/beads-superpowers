...
UserPromptSubmit 钩子在每条消息时提醒智能体哪个技能适用于哪个任务：
...
**触发条件：** npx 安装后，或技能未激活时。

在 `.claude/settings.json` 中注册 SessionStart 钩子，使技能自动激活。插件的 session-start 钩子自动检测 `bd setup claude` 钩子，并跳过重复的 `bd prime` 调用。
