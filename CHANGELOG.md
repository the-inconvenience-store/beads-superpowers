...
- **Public docs are README-first and English-only.** The deleted MkDocs site is no longer referenced from README, example-workflow, or CLAUDE.md; README now carries the user guide, skill index, workflow overview, and install paths. The no-op Chinese docs guard and README language toggle are removed rather than preserving a translation lane with no translated files.
